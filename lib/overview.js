...
// Runs the same HogQL queries against PostHog and assembles the identical
// `_ovData` shape, so a Vercel Cron can precompute it and the dashboard can read
// a warm, keyless copy instantly instead of firing heavy queries per visit.
//
// Bucket-aware: accepts view = 'overall' | 'influencer' | 'perf'. For 'overall'
// every bucket fragment is '' so the SQL is byte-identical to the pre-bucket
// version. For influencer/perf it injects the SAME attribution predicates the
// client uses (bucketExprSQL / bucketEmailFilter), so Influencer & Perf now load
// from the edge cache instead of firing ~40 client-side queries per open.
//
// IMPORTANT: the SQL below mirrors the fallback path in index.html. If you change
// a query there, change it here too (they must stay in lock-step or numbers drift).
...
// ── Attribution bucket engine — verbatim port of index.html ──────────────────
// SYNC: mirror bucketExprSQL()/bucketPersonSet()/bucketEmailFilter() in index.html.
const BUCKET_KW_CAMPAIGN = '23951249428';
// Bucket source/medium prefer the IMMUTABLE first-touch $initial_utm_* and only
// fall back to the mutable person.properties.source/medium (which a later 'direct'
// visit can overwrite, misrouting real Meta/Google paid users into Influencer).
// SYNC: identical strings in index.html.
const BKT_SRC = `lower(if(argMaxIf(toString(person.properties.$initial_utm_source), timestamp, person.properties.$initial_utm_source!='')!='', argMaxIf(toString(person.properties.$initial_utm_source), timestamp, person.properties.$initial_utm_source!=''), argMaxIf(toString(person.properties.source), timestamp, person.properties.source!='')))`;
const BKT_MED = `lower(if(argMaxIf(toString(person.properties.$initial_utm_medium), timestamp, person.properties.$initial_utm_medium!='')!='', argMaxIf(toString(person.properties.$initial_utm_medium), timestamp, person.properties.$initial_utm_medium!=''), argMaxIf(toString(person.properties.medium), timestamp, person.properties.medium!='')))`;
...
function bucketEmailFilter(view, emailCol, scanDays) {
  ...
  return ` AND ${emailCol} IN (
    ...
    FROM events WHERE person_id IN (${bucketPersonSet(view, scanDays)}) GROUP BY person_id) `;
}
// Derived-table person filter for event queries. MUST be used (not a per-event
// classifier) so signups/payments bucket the SAME way counts/sources do — a
// per-event test misclassifies the ~96% of signups with no source at signup time.
function bucketPersonFilter(view, scanDays) {
  if (view === 'overall') return '';
  return ` AND person_id IN (${bucketPersonSet(view, scanDays)}) `;
}
...
async function computeOverview(range, env, cust, view) {
  ...
  const evRangeInterval = `${scanDays} DAY`;

  // Bucket fragments — all '' for Overall (SQL then identical to pre-bucket path).
  const wantB = view === 'perf' ? 'perf' : 'infl';
  const countsBktCol      = view === 'overall' ? '' : `, ${bucketExprSQL('_bs','_bm','_bc')} AS _bkt`;
  const countsBucketOuter = view === 'overall' ? '' : `WHERE _bkt='${wantB}'`;
  const ovEvBkt           = view === 'overall' ? '' : ` AND ${bucketExprSQL('_bs','_bm','_bc')}='${wantB}'`;
  ...
  const counts = await runHogQL(`
          ...
          FROM (
            SELECT person_id, su, plan, status${countsBktCol}
            FROM (
              SELECT person_id,
                ...
                ${BKT_SRC} AS _bs,
                ${BKT_MED} AS _bm,
                argMaxIf(toString(person.properties.$initial_utm_campaign), timestamp, person.properties.$initial_utm_campaign!='') AS _bc
              FROM events
  ...
  const [signups, payments, revenue, sources, growth, dailyPay, influencers, pgSubscribers] = await Promise.all([
    runHogQL(`
          SELECT email, name, plan, created_at FROM (
            ...
            GROUP BY person_id
          ) WHERE email NOT LIKE '%@nextraise.ai' AND email NOT LIKE '%@creditdharma.in' AND email!=''${ovEvBkt}
    ...
    runHogQL(`
          ...
          WHERE event='payment_completed'
            ...
            AND person_id NOT IN COHORT 278743${bucketPersonFilter(view, 400)}
    ...
    runHogQL(`
          ...
          WHERE status='completed'
            ...
            AND lower(user_email) NOT LIKE '%test%'${bucketEmailFilter(view, 'user_email', scanDays)}`),
    runHogQL(`
          SELECT source, medium, count() AS c,
                 countIf(plan LIKE '%pro%' AND status='active') AS conv
          FROM (
            SELECT person_id, source, medium, plan, status${countsBktCol}
            FROM (
              SELECT person_id,
    ...
    runHogQL(`
          SELECT campaign, count() AS c,
                 countIf(plan LIKE '%pro%' AND status='active') AS conv
          FROM (
            SELECT person_id, campaign, plan, status FROM (
              SELECT person_id,
    ...
    runHogQL(`
          ...
          WHERE email NOT LIKE '%@nextraise.ai'
            ...
            AND lower(email) NOT LIKE '%test%'${bucketEmailFilter(view, 'email', scanDays)}`).catch(() => ({ results: [] })),
  ]);
