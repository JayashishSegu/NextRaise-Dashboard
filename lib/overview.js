...
async function computeOverview(range, env, cust, view) {
  ...
  const [signups, payments, revenue, growth, dailyPay, pgSubscribers, todayPay, sameWin] = await Promise.all([
    ...
    runHogQL(`
          ...
          FROM events
          WHERE event = 'signup_complete'
            AND toDate(toTimeZone(timestamp,'Asia/Kolkata')) >= toDate('${_swPrevStart}')
  ...
  return {
    ...
    // Prior-window signup total. The query that would populate this was never
    // added, so it stays null and the client keeps using its existing
    // counts-based comparison (newWeekPrev / newYesterday) exactly as before.
    prevSignups: null,
    dailyPay:    dailyPay.results    || [],
    todayPay:    todayPay.results   || [],   // [day, cnt, payers, inr, usd] for yest+today (IST)
    sameWin:     (sameWin.results || [])[0] || null,   // [cur, prev] signed-up-AND-paid in window
