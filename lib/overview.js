...
async function computeOverview(range, env, cust, view) {
  ...
  const [signups, payments, revenue, growth, dailyPay, pgSubscribers, todayPay, sameWin, payersBySrc, visitorsBySrc, pgSignupTotals, pgRevenueTotals, pgConvPresets] = await Promise.all([
    runHogQL(`
    ...
    // Daily signup counts from postgres.users — the auth DB, ground truth
    // for account creation. Was the PostHog signup_complete event, which
    // under-counted by ~15-20% (ad-blockers, iOS Safari, OAuth redirects,
    // users who bailed before the event fired). Hero + growth chart now
    // reflect actual accounts created; the acquire query still uses events
    // because that is where source/medium lives per person.
    runHogQL(`
          SELECT toString(toDate(toTimeZone(created_at,'Asia/Kolkata'))) AS day,
            count() AS c
          FROM postgres.users
          WHERE created_at >= toDateTime('${rangeStart} 00:00:00')
            AND created_at <  toDateTime('${rangeEnd} 00:00:00')
            AND email NOT LIKE '%@nextraise.ai'
            AND email NOT LIKE '%@creditdharma.in'
            AND lower(email) NOT LIKE '%test%'${bucketEmailFilter(view, 'email')}
          GROUP BY day ORDER BY day LIMIT 400`, 'nr-overview:growth'),
    runHogQL(`
          SELECT toString(toDate(toTimeZone(created_at,'Asia/Kolkata'))) AS day,
            count() AS cnt,
            sumIf(amount, currency='INR') AS inr,
    ...
    runHogQL(`
      ...
      .catch((e) => { if (e instanceof PostHogBudgetError) throw e; return { results: [] }; }),
    // Payers-in-range grouped by acquisition source/medium. Distinct payers
    // regardless of when they signed up. This is what reconciles the Acquisition
    // card's "paid" totals with the Payers · ARPU tile (revPayers): a person
    // who paid in-range but signed up before the range still counts here,
    // whereas the acquire conv_range only counts new signups who became Pro.
    // Postgres-side read is tiny (payments filtered to range); the persons
    // scan is the cost, already paid by other queries in this compute.
    runHogQL(`
          SELECT p.source, p.medium, uniqExact(pay.user_email) AS payers
          FROM postgres.payments pay
          JOIN (${PERSONS_SUB}) p ON lower(p.email) = lower(pay.user_email)
          WHERE pay.status='completed'
            AND pay.created_at >= toDateTime('${rangeStart} 00:00:00')
            AND pay.created_at <  toDateTime('${rangeEnd} 00:00:00')
            AND pay.user_email NOT LIKE '%@nextraise.ai'
            AND pay.user_email NOT LIKE '%@creditdharma.in'
            AND lower(pay.user_email) NOT LIKE '%test%'${bucketEmailFilter(view, 'pay.user_email')}${bktWhere}
          GROUP BY p.source, p.medium
          ORDER BY payers DESC LIMIT 200`, 'nr-overview:payersBySrc')
      .catch((e) => { if (e instanceof PostHogBudgetError) throw e; return { results: [] }; }),
    // Visitors-in-range grouped by acquisition source/medium. Distinct persons
    // who fired at least one $pageview during the range. Event-name-filtered
    // scan on '$pageview' only; the persons join is already cheap. This is the
    // top-of-funnel figure the Acquisition card needs to show visits → signups
    // → paid in one row.
    runHogQL(`
          SELECT p.source, p.medium, uniq(e.person_id) AS visitors
          FROM (
            SELECT person_id FROM events
            WHERE event = '$pageview'
              AND timestamp >= toDateTime('${rangeStart} 00:00:00')
              AND timestamp <  toDateTime('${rangeEnd} 00:00:00')
              AND person_id NOT IN COHORT 278743
            GROUP BY person_id
          ) e
          JOIN (${PERSONS_SUB}) p ON p.id = e.person_id
          WHERE NOT (${INTERNAL_EMAIL})
            AND lower(email) NOT LIKE '%test%'${bktWhere}
          GROUP BY p.source, p.medium
          ORDER BY visitors DESC LIMIT 200`, 'nr-overview:visitorsBySrc')
      .catch((e) => { if (e instanceof PostHogBudgetError) throw e; return { results: [] }; }),
    // Current and prior window signup totals from postgres.users (ground truth
    // for account creation, ties back to the growth query above). Emitted in
    // one row [cur, prev] so the client has a delta baseline that reconciles
    // exactly with the hero. Prior window is the same equal-length window
    // immediately before the selected range; for 'all' _swPrevStart equals
    // rangeStart so prev is 0 and no delta is shown.
    runHogQL(`
          SELECT
            countIf(created_at >= toDateTime('${rangeStart} 00:00:00')
                    AND created_at <  toDateTime('${rangeEnd} 00:00:00')) AS cur,
            countIf(created_at >= toDateTime('${_swPrevStart} 00:00:00')
                    AND created_at <  toDateTime('${rangeStart} 00:00:00')) AS prev
          FROM postgres.users
          WHERE created_at >= toDateTime('${_swPrevStart} 00:00:00')
            AND created_at <  toDateTime('${rangeEnd} 00:00:00')
            AND email NOT LIKE '%@nextraise.ai'
            AND email NOT LIKE '%@creditdharma.in'
            AND lower(email) NOT LIKE '%test%'${bucketEmailFilter(view, 'email')}`, 'nr-overview:pgSignupTotals')
      .catch((e) => { if (e instanceof PostHogBudgetError) throw e; return { results: [] }; }),
    // Current + prior window revenue in one row so the Revenue view of the
    // hero can render a like-for-like delta pill without a second call.
    // Same filters as the revenue query above; sums INR and USD separately
    // so the client can convert USD at its own live FX rate.
    runHogQL(`
          SELECT
            sumIf(amount, currency='INR' AND created_at >= toDateTime('${rangeStart} 00:00:00')
                    AND created_at <  toDateTime('${rangeEnd} 00:00:00')) AS inr_cur,
            sumIf(amount, currency='USD' AND created_at >= toDateTime('${rangeStart} 00:00:00')
                    AND created_at <  toDateTime('${rangeEnd} 00:00:00')) AS usd_cur,
            sumIf(amount, currency='INR' AND created_at >= toDateTime('${_swPrevStart} 00:00:00')
                    AND created_at <  toDateTime('${rangeStart} 00:00:00')) AS inr_prev,
            sumIf(amount, currency='USD' AND created_at >= toDateTime('${_swPrevStart} 00:00:00')
                    AND created_at <  toDateTime('${rangeStart} 00:00:00')) AS usd_prev,
            countIf(created_at >= toDateTime('${_swPrevStart} 00:00:00')
                    AND created_at <  toDateTime('${rangeStart} 00:00:00')) AS pay_prev
          FROM postgres.payments
          WHERE status='completed'
            AND created_at >= toDateTime('${_swPrevStart} 00:00:00')
            AND created_at <  toDateTime('${rangeEnd} 00:00:00')
            AND user_email NOT LIKE '%@nextraise.ai'
            AND user_email NOT LIKE '%@creditdharma.in'
            AND lower(user_email) NOT LIKE '%test%'${bucketEmailFilter(view, 'user_email')}`, 'nr-overview:pgRevenueTotals')
      .catch((e) => { if (e instanceof PostHogBudgetError) throw e; return { results: [] }; }),
    // Signup->Pro conversion per preset window, from postgres.users (the same
    // ground truth the hero uses). The acquire query still emits its own
    // event-based cs/cp columns for backwards compat, but the "Signup->Pro by
    // period" table reads THIS row instead so it never lags the hero.
    // Layout: [cs0, cp0, cs1, cp1, ...] one pair per CONV_PRESETS entry.
    runHogQL(`
          SELECT ${CONV_PRESETS.map((p, i) => { const b = gdrBounds(p.key);
            return `countIf(toDate(toTimeZone(u.created_at,'Asia/Kolkata'))>=toDate('${b.startDate}')
                     AND toDate(toTimeZone(u.created_at,'Asia/Kolkata'))<toDate('${b.endDate}')) AS cs${i}, `
                 + `countIf(toDate(toTimeZone(u.created_at,'Asia/Kolkata'))>=toDate('${b.startDate}')
                     AND toDate(toTimeZone(u.created_at,'Asia/Kolkata'))<toDate('${b.endDate}')
                     AND lower(u.email) IN (${PG_PRO_EMAILS})) AS cp${i}`;
          }).join(', ')}
          FROM postgres.users u
          WHERE u.email NOT LIKE '%@nextraise.ai'
            AND u.email NOT LIKE '%@creditdharma.in'
            AND lower(u.email) NOT LIKE '%test%'${bucketEmailFilter(view, 'u.email')}`, 'nr-overview:pgConvPresets')
      .catch((e) => { if (e instanceof PostHogBudgetError) throw e; return { results: [] }; }),
  ]);
  ...
  return {
    ...
    sources:     sourcesRows,
    // [source, medium, payers] — distinct payers in the range attributed by
    // the payer's source/medium. Client aggregates by channel and uses this
    // for the Acquisition card's "paid" totals so they reconcile with
    // Payers · ARPU. Conv-by-signup-cohort still lives in sources[3].
    payersBySource: payersBySrc.results || [],
    // [source, medium, visitors] — distinct persons with at least one
    // $pageview in the range, attributed by their source/medium. Top of the
    // Acquisition funnel: visits → signups → paid.
    visitorsBySource: visitorsBySrc.results || [],
    growth:      growth.results || [],
    // Current-window signup total from postgres.users. The hero big number
    // and delta read from here directly instead of summing the growth series,
    // so a transient failure on the growth query (empty results, partial
    // response) shows the real total instead of a hero-only 0.
    curSignups:  Number(((pgSignupTotals.results || [])[0] || [])[0]) || 0,
    // Prior-window signup total from postgres.users (matches the growth query
    // source of truth). Null for the 'all' preset where no comparable prior
    // window exists. sameWin[2] still carries the event-based prev count for
    // the Signed-up-and-paid cohort math, but the hero delta reads from here.
    prevSignups: _hasPrev ? (Number(((pgSignupTotals.results || [])[0] || [])[1]) || 0) : null,
    // Prior-window revenue for the Revenue hero tab's delta. INR and USD
    // kept separate so the client converts USD at its own live FX rate.
    // Null on 'all' where no comparable prior window exists.
    prevRevInr:    _hasPrev ? Number(((pgRevenueTotals.results || [])[0] || [])[2]) || 0 : null,
    prevRevUsd:    _hasPrev ? Number(((pgRevenueTotals.results || [])[0] || [])[3]) || 0 : null,
    prevPayCount:  _hasPrev ? Number(((pgRevenueTotals.results || [])[0] || [])[4]) || 0 : null,
    dailyPay:    dailyPay.results    || [],
    todayPay:    todayPay.results   || [],   // [day, cnt, payers, inr, usd] for yest+today (IST)
    sameWin:     (sameWin.results || [])[0] || null,   // [cur, prev, prevSignupsUniq]
    influencers: influencerRows,
    // Signup->Pro by preset window. Reads from the postgres.users-based
    // pgConvPresets aggregate so the table matches the hero and the DB truth
    // (was event-based signup_complete, undercounted by 15-20% because
    // ad-blockers/OAuth redirects drop the client event).
    convByPeriod: (function () {
      const pcRow = (pgConvPresets.results || [])[0] || [];
      return CONV_PRESETS.map((p, i) => {
        const s = pcRow.length
          ? (+pcRow[i * 2] || 0)
          : (+mergedCounts[13 + i * 2] || 0);
        const pr = pcRow.length
          ? (+pcRow[i * 2 + 1] || 0)
          : (+mergedCounts[13 + i * 2 + 1] || 0);
